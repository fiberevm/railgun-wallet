...
const generateUnshieldBaseToken = async (txidVersion, txs, networkName, toWalletAddress, relayAdaptParamsRandom, 
...
useDummyProof = false, sendWithPublicWallet) => {
    ...
    const chain = shared_models_1.NETWORK_CONFIG[networkName].chain;
    const transaction = await engine_1.RelayAdaptVersionedSmartContracts.populateUnshieldBaseToken(txidVersion, chain, txs, toWalletAddress, relayAdaptParamsRandom, useDummyProof, sendWithPublicWallet);
    if (useDummyProof) {
        return {
            ...transaction,
            from: exports.DUMMY_FROM_ADDRESS,
...
const transactionsFromERC20Amounts = async (proofType, erc20AmountRecipients, nftAmountRecipients, railgunWallet, txidVersion, encryptionKey, showSenderAddressToRecipient, memoText, networkName, broadcasterFeeERC20AmountRecipient, sendWithPublicWallet, relayAdaptID, useDummyProof, overallBatchMinGasPrice, progressCallback, originShieldTxidForSpendabilityOverride) => {
    ...
    const txBatches = await generateAllProofs(transactionBatch, railgunWallet, txidVersion, encryptionKey, useDummyProof, progressCallback, originShieldTxidForSpendabilityOverride);
    return txBatches;
};
...
const generateAllProofs = async (transactionBatch, railgunWallet, txidVersion, encryptionKey, useDummyProof, progressCallback, originShieldTxidForSpendabilityOverride) => {
    ...
    return transactionBatch.generateTransactions(prover, railgunWallet, txidVersion, encryptionKey, progressCallback, false, originShieldTxidForSpendabilityOverride);
};
