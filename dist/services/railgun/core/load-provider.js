...
const loadProviderForNetwork = async (chain, networkName, fallbackProviderJsonConfig, pollingInterval) => {
    ...
    const network = shared_models_1.NETWORK_CONFIG[networkName];
    const { proxyContract, relayAdaptContract, poseidonMerkleAccumulatorV3Contract, poseidonMerkleVerifierV3Contract, tokenVaultV3Contract, deploymentBlockPoseidonMerkleAccumulatorV3, deploymentBlock, publicName, supportsV3, } = network;
    if (!proxyContract) {
    ...
    await engine.loadNetwork(chain, proxyContract, relayAdaptContract, poseidonMerkleAccumulatorV3Contract, poseidonMerkleVerifierV3Contract, tokenVaultV3Contract, fallbackProvider, pollingProvider, deploymentBlocks, supportsV3);
};
/**
